    ...
    try:
        ...
        if value is not None:
            ...
            if 'payload' in kafka_message and kafka_message['payload'] is not None:
                ...
                elif 'op' in payload and payload['op'] == 'u':
                    if 'after' in payload and isinstance(payload['after'], dict):
                        emp_id = payload['after'].get('emp_id')
                        emp_name = payload['after'].get('emp_name')
                ...
                elif 'op' in payload and payload['op'] == 'd':
                    if 'before' in payload and isinstance(payload['before'], dict):
                        emp_id = payload['before'].get('emp_id')
                        emp_name = payload['before'].get('emp_name')

                        print('\n\nDelete occurred!')
                        print(f'Deleted Employee ID: {emp_id}')
                        print(f'Deleted Employee Name: {emp_name}')
                        print('---------------------------------------------')
